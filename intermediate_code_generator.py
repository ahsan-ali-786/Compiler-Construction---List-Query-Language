...
#         self.temp_count = 0
#         self.result_var = "result"  # standard place for stats results
...
#         method = getattr(self, f"visit_{nodetype}", None)
#         if not method:
#             raise Exception(f"No TAC handler for node type {nodetype}")
#         return method(node)

#     # ---------- Node handlers ----------
#     def visit_ListDecl(self, node):
#         # node.name, node.values (list of numbers)
#         self.instructions.append(('LIST', node.name, list(node.values)))
#         return node.name

#     def visit_FilterStmt(self, node):
#         # node.name (src), node.op, node.number
#         dest = self.new_temp()
#         self.instructions.append(('FILTER', dest, node.name, node.op, node.number))
#         # replace source with dest (in-place behaviour in high-level)
#         # but also emit assign to original name so subsequent statements can use it
#         self.instructions.append(('ASSIGN', node.name, dest))
#         return node.name

#     def visit_SortStmt(self, node):
#         dest = self.new_temp()
#         self.instructions.append(('SORT', dest, node.name, node.order))
#         self.instructions.append(('ASSIGN', node.name, dest))
#         return node.name

#     def visit_MapStmt(self, node):
#         # node.list_name, node.var, node.expr (Expr)
#         dest = self.new_temp()
#         # stringify expression using the map variable name so executor can eval it
#         expr_code = self.expr_to_code(node.expr, map_var=node.var)
#         self.instructions.append(('MAP', dest, node.list_name, node.var, expr_code))
#         self.instructions.append(('ASSIGN', node.list_name, dest))
#         return node.list_name

#     def visit_StatStmt(self, node):
#         # func, list_name
#         dest = self.result_var
#         self.instructions.append(('STAT', dest, node.func, node.list_name))
#         # keep result in 'result'
#         return dest

#     def visit_PrintStmt(self, node):
#         # node.target (id or 'result')
...
#         # op, list1, list2
#         dest = self.new_temp()
#         self.instructions.append(('SETOP', dest, node.op, node.list1, node.list2))
#         # assign back to left operand (convention) — but we won't modify original lists automatically
#         # instead produce temp and also print or assign to a name if needed
#         return dest

#     # ---------- Expression handling ----------
#     def expr_to_code(self, expr, map_var=None):
#         """
#         Convert Expr AST into an infix expression string.
#         If map_var provided, identifiers equal to that var are left as is.
#         Any other identifier is treated as variable name (should be allowed only map_var).
#         Numbers are rendered as-is (ints if integer value).
#         """
#         # Base cases: expr could be Expr with op None and left numeric or identifier
#         if expr is None:
#             return ""
#         if expr.op is None:
#             val = expr.left
#             if isinstance(val, (int, float)):
#                 # render integer without decimal if it's whole
#                 if isinstance(val, float) and val.is_integer():
#                     return str(int(val))
#                 return str(val)
#             # identifier
#             if isinstance(val, str):
#                 if map_var and val == map_var:
#                     return map_var
#                 # otherwise keep as identifier (could be other variable — semantic check should disallow)
#                 return val
#             # nested Expr?
#             return str(val)

#         # recursive case
#         left_code = self.expr_to_code(expr.left, map_var)
#         right_code = self.expr_to_code(expr.right, map_var)
#         return f"({left_code} {expr.op} {right_code})"

#     # Utility to pretty print current TAC
#     @staticmethod
