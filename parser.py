...
# class FilterStmt:
#     def __init__(self, name, op, number):
#         self.name = name
#         self.op = op
#         self.number = number
...
# class MapStmt:
#     def __init__(self, list_name, var, expr):
#         self.list_name = list_name
#         self.var = var
#         self.expr = expr

# class StatStmt:
#     def __init__(self, func, list_name):
#         self.func = func
#         self.list_name = list_name
...
# class SetOpStmt:
#     def __init__(self, op, list1, list2):
#         self.op = op
#         self.list1 = list1
#         self.list2 = list2
...
#             raise Exception(f"Expected value {value}, got {tok}")
...
#         # stats: mean nums
#         if tok[0] == 'KEYWORD' and tok[1] in (
#             'mean','sum','median','variance','std','min','max','count'
#         ):
#             return self.stat_stmt()

#         # print nums or print result
#         if tok == ('KEYWORD', 'print'):
#             return self.print_stmt()

#         raise Exception(f"Unexpected token in statement: {tok}")
...
#             values.append(float(self.match('NUMBER')[1]))
#             while self.accept('COMMA'):
#                 values.append(float(self.match('NUMBER')[1]))

#         self.match('RBRACK')
#         return ListDecl(name, values)

#     # filter nums > 4
#     def filter_stmt(self):
#         self.match('KEYWORD', 'filter')
#         list_name = self.match('ID')[1]
#         op = self.match('OP')[1]
#         number = float(self.match('NUMBER')[1])
#         return FilterStmt(list_name, op, number)

#     # sort nums asc
#     def sort_stmt(self):
#         self.match('KEYWORD', 'sort')
#         name = self.match('ID')[1]
#         order = self.match('KEYWORD')[1]  # asc or desc
#         return SortStmt(name, order)

#     # map nums x -> expr
#     def map_stmt(self):
#         self.match('KEYWORD', 'map')
#         list_name = self.match('ID')[1]
#         var = self.match('ID')[1]
#         self.match('ARROW')
#         expr = self.expr()
#         return MapStmt(list_name, var, expr)

#     # mean nums
#     def stat_stmt(self):
#         func = self.match('KEYWORD')[1]
#         list_name = self.match('ID')[1]
#         return StatStmt(func, list_name)

#     # print nums / print result / print nums union nums2
#     def print_stmt(self):
#         self.match('KEYWORD', 'print')

#         tok1 = self.match('ID') if self.current()[0] == 'ID' else self.match('KEYWORD', 'result')
#         target = tok1[1]

#         # set operations
#         if self.current()[0] == 'KEYWORD' and self.current()[1] in ('union','unique','intersection','difference'):
#             op = self.match('KEYWORD')[1]
#             list2 = self.match('ID')[1]
#             return SetOpStmt(op, target, list2)
...
#         if tok[0] == 'NUMBER':
#             return Expr(float(self.match('NUMBER')[1]))

#         if tok[0] == 'ID':
#             return Expr(self.match('ID')[1])
...
#         raise Exception(f"Unexpected factor: {tok}")
